...
echo "-------------------step1-------------------------------"
  ...
  $SED -Ei "s|^SF:$OBJ_DIR/(.+)$|SF:$Data_DIR/\1|g" "$LCOV_INFO"
  
  echo "$SED -Ei s|^SF:$BUILD_SRC_PATH/(.+)$|SF:$SRCROOT/\1|g $LCOV_INFO"
   echo "$SED -Ei s|^SF:$OBJ_DIR/(.+)$|SF:$Data_DIR/\1|g $LCOV_INFO"
...
exclude_data() {
  ...
  LCOV --remove "${LCOV_INFO}" "*xcworkspace/*" -d "${OBJ_DIR}" -o "${LCOV_INFO}" 
